...
**Phase 2.6・2.7 は 2026-09-03 に完了した（M2.7「どこでも利用可能」到達）。**
**両フェーズとも DoD 全項目を達成している。次の計画上の工程は Phase 2.8（見る手段と戻る手段）。**
...
挟むと定めており、**2.6 で配信オリジンと起動画面が、2.7 で入力手段が変わった。**
**数え直しの起点は 2026-09-03。2.8 へ進む前に、使ってみて出たことを優先する。**

**IMPORTANT: 2.7 の DoD 6項目は、1つも机上で確かめられなかった。**
...
- **IMPORTANT: 構造編集の入口はキーボードにしか無かった**（設計書 7.4 で
  `mind-elixir` の右クリックメニューとツールバーを切ってあるため）。**指しか無い端末では
  読むことしかできなかった。** 狭い画面用の編集バーを足して塞いだ（設計書 7.2.1）。
  並べる操作は `src/app/edit-bar.ts`。**この一覧が嘘をつくとその端末では操作が失われる**ので、
  `keymap.ts` などと同じく描画層にありながら自動テストを持つ
