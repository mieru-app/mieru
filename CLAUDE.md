...
**Phase 2.6（置き場所）は 2026-09-03 に完了した（M2.6 到達）。次は Phase 2.7（スマートフォン対応）。**
**ただし計画（5章）はマイルストーン間に実利用期間を挟むと定めている。**
2.6 で起動画面と配信オリジンが変わったため、**実利用の数え直しはここが起点**になる。
2.7 へ進む前に、まず使ってみて出たことを優先する。
