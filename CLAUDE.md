...
**Phase 1（編集機能）完了。実機動作を確認済み。Phase 2 着手前。**
...
Phase 2 のうちテンプレート（2-10）と全文検索（2-3）は、実際の使い方が分かるまで要件を決められない。
先に進めてよいのは名称・アイコン・PWA化・配信・性能試験である。

名称は **Mieru** で確定（docs/design.md 12.5）。Phase 2 の配信先は **GitHub Pages**（同 8.6）。
