...
**Phase 2.5（言葉と導線）は 2026-09-02 に完了した。次は Phase 2.6（GitHubStore）。**
...
**2.6-1（事前検証）は読み取り側だけ済んでいる。** 結果は `docs/github-api-verification.md`。
Contents API の `sha` は git の blob ハッシュそのもので、内容だけで決まることを実測した。
**楽観ロックが実際に効くかは未確定で、利用者の Fine-grained PAT が要る。**
実装前に決める事項4件も同メモ4章にある。とくに **GitHub のセカンダリ制限
「内容を作る要求は 500/時」**があるため、自動保存の 800ms を GitHub にそのまま使えない。
