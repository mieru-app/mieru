...
**Phase 2.8（見る手段と戻る手段）は 2026-09-04 に5タスクすべて実装した。**
フェーズ定義と完了条件は `docs/project-plan.md` の3〜4章。改修要望への対応は
同 Phase 2.5〜2.10（2026-09-02 追加）。

**IMPORTANT: 2.8 の DoD 2項目は、まだ実機で確かめていない。**
`Source.tsx`・`HistoryPanel.tsx` は描画層であり自動テストを持たない。
判断だけを `view-mode.ts`・`diff.ts`・`history-policy.ts` へ出して検証してある。
**確かめる項目は計画書 Phase 2.8 の「実機で確かめること」にある。**
**2.9 へ進む前にそれを通すこと。**
...
**数え直しの起点は 2026-09-03。使ってみて出たことを、次の工程より優先する。**
...
触るときに読み込まれる）。要点は「`mind-elixir` は指では何もできない」ことと、
**入力手段が無いことは画面を見ても分からない**ことである。
