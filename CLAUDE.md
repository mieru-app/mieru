...
**M2.10「読める・戻せる」は未到達**（Phase 2.9・2.10 が残る）。

**IMPORTANT: Phase 2.9 は実装が終わり、実機確認が1点だけ残っている（2026-09-04）。**
2.9-2（親子の反転、`Ctrl+Shift+↑`）と 2.9-3（アウトラインの掴み替え）を入れた。
**「実装した」を「できた」と書かないこと。**

- ~~`Ctrl+Shift+↑` がブラウザや OS に先に取られないか~~
  **2026-09-04 に実機で確認。取られない**
- **残り: アウトラインのグリップが指で掴めるか**（`touch-action` を切ったのは
  グリップだけであり、当たり判定は広げたが指の太さは机上で測れない）

**掴み替えは長押しではなくグリップ方式にした。** 一覧は指で縦に送る面であり、
長押しで掴む方式にすると送る操作と競合する。規則は設計書 7.4（F-21a）。
